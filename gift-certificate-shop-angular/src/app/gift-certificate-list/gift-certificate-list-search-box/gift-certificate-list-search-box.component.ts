...
@Component({
  selector: 'app-gift-certificate-list-search-box',
  templateUrl: './gift-certificate-list-search-box.component.html',
  styleUrls: ['./gift-certificate-list-search-box.component.scss']
})
export class GiftCertificateListSearchBoxComponent implements OnInit {

  tagsSelectShowSearch = true;
  tagsSelectShowError = false;
  tagsSelectShowAll = true;
  tagsSelectShowStatus = true;
  tagsSelectPlaceholder = "pages.gift-certificate.list.search-box.tags-select-placeholder";

  sortSelectShowSearch = false;
  sortSelectShowError = false;
  sortSelectShowAll = false;
  sortSelectShowStatus = false;
  sortSelectPlaceholder = "pages.gift-certificate.list.search-box.sort-select-placeholder";
  ...
  loadSortItemsTranslate() {
    for (let sortItem of this.sortItems) {
      this.translateService.stream("pages.gift-certificate.list.search-box.sorts." + sortItem.name).subscribe(
        translate => {
